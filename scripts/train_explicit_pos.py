'''
A simple explicit position prediction script.
'''
...
def get_ins_and_outs(inputs, outputs, num_to_save = 1, t_in = 3, t_out = 3, **loss_params):
    ...
    '''
    num_points = inputs['positions_parsed'].get_shape().as_list()[1] / (3 * (t_in + t_out))
    input_pos = inputs['positions_parsed'][:num_to_save, : num_points * 3 * t_in]
