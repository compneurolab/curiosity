...
if __name__ == '__main__':
    ...
    get_oimg = valid_targets_dict['valid0']['targets']['oimg']
    ...
    for ex in xrange(valid_targets_dict['valid0']['num_steps']):
        ...
        print('Getting data')
        images, actions, objects, object_data, \
                oimg, oact, opos = sess.run([get_images, 
            get_actions, 
            get_objects,
            get_object_data,
            get_oimg,
        ...
        if USE_TRUE:
            inputs_file = os.path.join(SAVE_DIR, 
                    'true_inputs_'+exp_id+'_'+str(ex)+'.pkl')
        else:
            inputs_file = os.path.join(SAVE_DIR,
        ...
        results = {'pred_poses': predicted_poses,
                ...
                'ex_acted': acted}
        if USE_TRUE:
            results_file = os.path.join(SAVE_DIR, 
                    'true_results_'+exp_id+'_'+str(ex)+'.pkl')
        else:
            results_file = os.path.join(SAVE_DIR, 
                    'results_'+exp_id+'_'+str(ex)+'.pkl')
