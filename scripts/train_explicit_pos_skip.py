'''
A simple explicit position prediction script, this time skipping some time.
'''
...
def get_ins_and_outs(inputs, outputs, num_to_save = 1, seq_len = 6, t_out = 3, **loss_params):
    ...
    num_points = inputs['positions_parsed'].get_shape().as_list()[1] / (3 * seq_len)
    all_but_outputs = inputs['positions_parsed'][:num_to_save, : - num_points * 3 * t_out]
    output_pos = outputs['tv'][:num_to_save]
    pred = outputs['pred'][:num_to_save]
    retval = {'inpos' : input_pos, 'outpos' : output_pos, 'pred' : pred, 'allbut' : all_but_outputs}
...
def get_loss_for_val(inputs, outputs, num_channels = 3, threshold = None, **loss_params):
   	return {'loss' : modelsource.l2_loss_fn(outputs, inputs['positions_parsed'])}
...
params = {
	'save_params' : {
        ...
        'save_to_gfs': ['inpos', 'outpos', 'pred', 'allbut'],
        'cache_dir' : '/media/data/nhaber',
	},

	'model_params' : {
		'func' : modelsource.position_only_with_skip,
		'cfg' : cfg_50pts,
        'T_in' : T_in,
        'T_out' : T_out,
        'skip' : SKIP,
