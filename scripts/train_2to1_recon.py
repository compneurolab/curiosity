...
n_classes = 768
buckets = 255
CFG = [ modelsource.cfg_mom_concat(n_classes, use_cond=False, method='concat'),
        modelsource.cfg_mom_flat_concat(n_classes, use_cond=False, method='concat'), 
        modelsource.cfg_mom_flat_bypass(n_classes, use_cond=False, method='concat'), 
        modelsource.cfg_mom_flat_bypass(n_classes, use_cond=False, method='concat',
            nonlin='crelu')]
USE_VEL = [False, False, False, False]
USE_SEG = [False, False, False, False]
...
def grab_all(inputs, outputs, bin_file = BIN_FILE, 
    ...
    retval['loss'] = modelsource.softmax_cross_entropy_loss_depth( 
...
loss_params = [{
    ...
    'loss_per_case_func' : modelsource.softmax_cross_entropy_loss_depth,
