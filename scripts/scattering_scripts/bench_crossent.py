'''
Cross entropy loss 1 to 1 benchmark for scatter dataset
'''
	...
	for w in widths:
		for d in depths:
			...
			if not os.path.exists(CACHE_DIR):
				os.mkdir(CACHE_DIR)
			cfg = model_cfg_gen(depth = d, width = w, drop_keep = drop_keep, num_classes_per_dim = NUM_CLASSES)
			print(cfg)
