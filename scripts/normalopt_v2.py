...
num_tries = 10
def main(ind, dbname, colname, srcdir, savedir, gpu=0, script='normal_encoder_opt_source.py', decayrate=0.95, decaystep=100000, num_train_steps=2048000):
    for n in range(num_tries):
        for learningrate in [5., 1., .75, .5]:
            ...
            if conn[dbname][colname].find({'experiment_id': experiment_id}).count() > 10:
                ...
                break
            cmd_tmp = """CUDA_VISIBLE_DEVICES=%d python %s/curiosity/curiosity/sandbox/%s %s %s %s --seed %d --learningrate=%f --savedir=%s --decaystep=%d --decayrate=%f --num_train_steps=%d"""
            cmd = cmd_tmp % (gpu, srcdir, script, dbname, colname, experiment_id, seed, learningrate, savedir, decaystep, decayrate, num_train_steps)
            print('CMD: %s' % cmd)
            os.system(cmd)
            if conn[dbname][colname].find({'experiment_id': experiment_id}).count() > 10:
