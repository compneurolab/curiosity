...
parser.add_argument('--scenelen', default = 256 * 1024, type = int)
...
do_torque = False
obj_size = .4
scene_len = args['scenelen']
image_shape = image_scale =  (128, 170)
RENDER1_HOST_ADDRESS = '10.102.2.161'
force_scaling = args['forcescaling']
save_filename = '/media/data2/nhaber/swtch' + str(int(force_scaling)) + '_' + tasknum + '.hdf5'
room_dims = (5., 5.)
state_time_length = 2
...
data_params = {
			...
			'termination_condition' : environment.obj_not_present_termination_condition
                ...
                'provider_params' : {
                        'batching_fn' : lambda hist : data.uniform_experience_replay(hist, history_len, my_rng = my_rng, batch_size = batch_size, allow_repeats = True),
                        'capacity' : 5,
